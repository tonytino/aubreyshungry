...
const HOUSE_RULES =
  ...
  `before changing code. If the task touches ANY food content (plans, recipes, snacks, shopping ` +
  `lists, seed data), read docs/agents/dietary-safety.md FIRST — 100% gluten-free, absolutely no ` +
  `cashews or pistachios, anti-inflammatory as the objective function. No new dependencies, no ` +
  `\`any\`, no \`process.env\` outside app/env.ts, tests for new behavior, and \`pnpm preflight\` ` +
  `must stay green.`
...
const reviewerPrompt = (item, round) =>
  ...
  `- Dietary safety (MANDATORY for any food content; any finding is automatically a blocker): ` +
  `gluten in any form, cashew/pistachio in any form (watch cashew-cream vegan recipes, curry ` +
  `pastes, soy sauce), missing cross-contact notes, pro-inflammatory drift (docs/agents/dietary-safety.md).\n` +
  `- Correctness: logic bugs, edge cases, wrong assumptions, broken behavior.\n` +
  `- Security: injection, secret exposure, workflow permissions, unsafe input.\n` +
...
const addressPrompt = (item, verdict) =>
  ...
  `justification (why it is not a real problem). Dietary-safety findings are never rebutted on ` +
  `convenience grounds — when uncertain whether an ingredient is safe, exclude it. Make file ` +
  `edits only — no git.\n\n` +
  `## Original task\n${spec(item)}\n\n## Reviewer findings\n` +
  '```json\n' +
