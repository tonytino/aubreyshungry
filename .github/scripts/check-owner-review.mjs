...
// Owner-review detector — Layer 2 of the guardrail (docs/agents/governance.md).
...
// `safe:human` — i.e. an agent tried to self-classify a dietary-safety/cost/
// legal/security/data-loss/process-config change as `safe:agent`
...
// (destructive SQL, disclaimer copy, forbidden-ingredient terms).
//
...
// git or the filesystem. Tests: tests/unit/check-owner-review.test.ts. The PR
// labels/diff are read from env in main() — never inline argv — so a hostile
// branch name / label can't inject.
//
// Local/manual usage:
//   BASE_SHA=origin/main OWNER_REVIEW_LABELS=safe:agent node .github/scripts/check-owner-review.mjs
// or without a branch, via env overrides:
//   OWNER_REVIEW_FILES=$'app/env.ts' OWNER_REVIEW_LABELS=safe:human node .github/scripts/check-owner-review.mjs
...
export function parseUnifiedDiff(diffText) {
  ...
  // file header. `--- `/`+++ ` are headers only BEFORE the first `@@` of a file;
  // once inside a hunk, `+`/`-` lines are content until the next `diff --git`.
  let inHunk = false;
 ...
 *
 * @param {{ file: string, side: "add" | "del", text: string }[]} entries
...
function collectDiff() {
  ...
  // and header matching. Diffing from the merge-base is three-dot-equivalent
  // scoping (only this branch's own changes).
...
function main() {
  ...
  console.error(
    `\nThis PR touches an owner-gated surface (dietary safety / cost / legal / security / data-loss / process-config). Relabel it \`${OWNER_LABEL}\` (never \`safe:agent\`): only ${OWNER_HANDLE} can approve and merge it, a human clicks merge, and there is NO bypass label. See docs/agents/governance.md.`
  );
  process.exit(1);
}
