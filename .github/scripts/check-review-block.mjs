...
//   3. As part of (2a): after stripping HTML comments (a template's
...
/**
 * Strip `<!-- ... -->` HTML comments so a template placeholder comment isn't
 ...
 * Pure and unit-testable — see tests/unit/check-review-block.test.ts.
 *
...
export function validateReviewBlock(body) {
  ...
  // Reject an empty / bare-placeholder section (e.g. only a `-` from a template).
...
if (process.argv[1]?.endsWith("check-review-block.mjs")) {
  main();
}
