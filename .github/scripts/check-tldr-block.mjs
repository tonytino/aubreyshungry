...
// Ported and adapted from the sibling repo aubreyslist (there the TL;DR feeds
// a Slack merge feed; here it exists purely to make PRs reviewable at a
// glance — the owner reads the TL;DR before deciding how deep to go).
//
// Validates that a PR body carries a plain-language TL;DR — the 1–3 sentences
// a non-engineer can understand. Like the adversarial-review gate, this is a
// forcing function: it can't judge whether the prose is actually jargon-free,
// only that the author wrote *something* real in the section.
//
// ── Validity contract ────────────────────────────────────────────────────────
...
// job passes the title via env, and how check-review-block.mjs reads its
// input).

const REMEDY =
  "Add a `## TL;DR` section at the top of the PR body with 1–3 plain sentences " +
  "a non-engineer can understand — what changed and why it matters. The section " +
  "is provided by .github/pull_request_template.md.";
...
/**
 * Find the TL;DR section in a PR body and return its content: the lines after
 * the TL;DR heading, up to the next heading of same-or-shallower level (or end
 * of body), with HTML comments stripped. Heading matches inside fenced code
...
if (process.argv[1]?.endsWith("check-tldr-block.mjs")) {
  main();
}
