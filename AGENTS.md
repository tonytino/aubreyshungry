...
A public weekly meal-plan digest — gluten-free, anti-inflammatory, and free of
cashews and pistachios — built **agent-first**. Start with
`docs/product/overview.md` for the product vision and `docs/agents/domain.md`
for the domain model.

**Before touching ANY food content (plans, recipes, snacks, shopping lists,
seed data), read `docs/agents/dietary-safety.md` — the golden rules.**
...
2. **If `TEMPLATE.md` exists in this repo**, read it before doing anything else — it means you are modifying the aubreyshungry template itself, not a project built from it. The design principles there govern every decision.
...
| Weekly plan generation, regeneration, cron    | `docs/agents/generation.md`   |
...
- **GOLDEN RULE: all food content is 100% gluten-free.** No exceptions,
  including seed data, test fixtures, and examples. See `docs/agents/dietary-safety.md`.
- **GOLDEN RULE: absolutely no cashews or pistachios, in any form or derived
  product** — a household member has a life-threatening allergy. When
  uncertain whether an ingredient qualifies, exclude it. See `docs/agents/dietary-safety.md`.
- **GOLDEN RULE: meal plans optimize for anti-inflammatory nutrition.** It is
  the objective function, not a nice-to-have. See `docs/agents/dietary-safety.md`.
- **Owner-gated changes are `safe:human`, never `safe:agent`** — dietary
