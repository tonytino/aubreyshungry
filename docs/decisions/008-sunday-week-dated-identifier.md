...
- **Amendment 2 to ADR-007 — the `[YYYY-Www]` parameter is replaced by an interactive prompt.** `/generate-week` takes no argument. The skill finds the latest `weekStart` under `content/weeks/`, then asks the owner (AskUserQuestion) to choose between regenerating that week and generating the next one (`+7 days`). With no weeks on disk it offers the current Mountain-Time week (the Sunday on or before today) and the one after it. It never guesses.
